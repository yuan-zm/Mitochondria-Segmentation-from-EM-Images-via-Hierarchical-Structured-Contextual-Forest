function [proMit,resL] = poolACpre(imgs,pros,prosp,procz,mdl,opts)
...
k=1; gtRidus = floor(opts.gtWidth{opts.stage}/ 2);
...
parfor i = 1:length(z) %pdsz + 1: 2:numPic -pdsz  % here i is the img num in column
    ...
    [ppatch ,phpatch] = getProPatch(prosp,xyz,opts);
    proFtsp = extProFeat(ppatch,phpatch,opts);ppatch=[];phpatch=[];
    [ppatch ,phpatch] = getProPatch(procz,xyz,opts);
    proFtcz = extProFeat(ppatch,phpatch,opts);ppatch=[];phpatch=[];
%     if( opts.useMoreContext == 1 && opts.stage == 2)
%     imgFt = imgFt(:,opts.finalAppDel); proFt = proFt(:,opts.finalProDel); end
    
    [pre,~] = forestApply( [imgFt,proFt,proFtsp,proFtcz], mdl );
    allPre{i}=pre;
    imgFt = []; proFt = [];%#ok
