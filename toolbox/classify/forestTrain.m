...
while( k < K )
  ...
  if(discr)
      [hs1,hsn{k}]=feval(discretize,hs1,H); hs1=uint32(hs1);
       lid = randperm(size(hs1,2),1);lids(k)=lid;hs1 = hs1(:,lid);
  end
...
tree=struct('fids',fids(K),'thrs',thrs(K),'child',child(K),...
  'distr',distr(K,:),'hs',hsn,'count',count(K),'depth',depth(K),'lids',lids(K));
end
