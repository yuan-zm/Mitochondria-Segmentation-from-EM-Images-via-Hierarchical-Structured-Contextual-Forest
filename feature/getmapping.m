...
%  LBP codes in a neighbourhood of SAMPLES sampling ：SAMPLES邻接采样数
%  points. Possible values for MAPPINGTYPE are
%       'u2'   for uniform LBP 等价LBP
%       'ri'   for rotation-invariant 旋转不变LBP
%       'riu2' for uniform rotation-invariant LBP. 等价旋转不变
%
%  Example:
%       I=imread('rice.tif');
...
%
%mapping.samples=8邻接lbp，mapping.num最大lbp值，mapping.table每一中lbp算子的最小值描述取值
...
if strcmp(mappingtype,'u2') %Uniform 2
  ...
  for i = 0:2^samples-1
    ...
    a=bitshift(i,1,'uint8');%位函数
    b=bitget(i,samples);%参数说明：x是要被转换为2进制的数，输出从低位低 m 位到第 n 位的数字
    j = bitset(bitshift(i,1,'uint8'),1,bitget(i,samples)); %rotate left 将指定的数据位设置为1，从右边向左
